...
日期：2026-08-02（公共三平台 CI 复验：2026-08-04；最终 Windows 候选本机复验：2026-08-05）
...
| 下载候选 Windows 隔离安装烟测    | 安装/卸载退出码 0；冷启动 3.23 秒；残留进程 0         |
...
| 当前源码对应的 Windows 安装包     | CI run `30933901524` 从提交 `2da293f` 构建 0.7 NSIS，并完成运行时完整性、安装烟测和已安装应用 E2E；下载后的安装包为 181,391,882 字节，SHA-256 `C86654663A807DAB2D7543A588A1009773E1857E56A12B7F6664F3755A64AA59`，重算值与随包 `SHA256SUMS.txt` 一致，Authenticode 状态为 `NotSigned`。2026-08-05 对同一下载文件执行隔离本机复验：安装/卸载退出码 0、冷启动 3.23 秒、窗口标题正确、无需强杀、关闭后残留进程 0；已安装应用核心 E2E 9/9，通过后安装目录、用户数据与产品进程均无残留。 | 当前公共 CI 未签名候选及本机自动化复验通过                                        |
...
| 当前源码对应的 Linux x64 AppImage | 公共 CI run `30933901524` 在 Linux x64 原生 runner 完成 JDK 21 Java 调试、AppImage 构建、运行时完整性和已打包应用 E2E；下载文件为 213,150,044 字节，SHA-256 `8D6E63BD929052DFE2BC8CD76E83853569D7060EA31CAEE37086CEEA3D628469`，与清单一致。                                                                                                                                                                                                                                        | 满足“Linux 至少可构建”及公共原生 CI 自动化证据；仍非公开 Release/独立人工设备证据 |
| 当前源码对应的 macOS 产物         | CI run `30933901524` 在 `macos-15` ARM64 以 JDK 21 完成真实 Java 调试，生成 DMG/ZIP，验证 JDT LS、Java Debug Server、`node-pty` 原生 helper 的格式与执行权限，并完成已打包应用 E2E；DMG 与 ZIP 的重算 SHA-256 均与清单一致。                                                                                                                                                                                                                                                        | 当前提交的原生自动化已满足；仍未签名/notarize，也没有独立人工设备证据             |
