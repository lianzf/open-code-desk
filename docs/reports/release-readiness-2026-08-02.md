...
日期：2026-08-02（当前候选公共三平台 CI 制品复验：2026-08-09；本机 Windows 复验：2026-08-08）
审计对象：本地 `release/0.7.0-alpha.1` 工作树  
结论：核心产品闭环与本地质量门禁已通过；当前产品提交 `a3471ca14f883845b7a6bc3bacfef25dfa65d5ba` 的三平台公共 CI run `31295938442` attempt 1 全部成功，四个必需 artifact 已下载并完成 19/19 清单与哈希核验。按 2026-08-09 调整后的范围，该候选满足受控内部 Alpha 交付；正式签名、真实 Provider 与独立设备验收仍缺失，正式发布明确延期。
...
### 前一候选的本机精确 Windows 证据（提交 07ac062）
...
| 当前源码对应的 Windows 安装包     | 当前提交 `a3471ca` 的 CI run `31295938442` attempt 1、Windows job `93201306020` 完成构建、安装烟测、已安装 E2E 与 artifact 上传；公共 NSIS 为 182,383,661 字节，SHA-256 `EAE087D6DBE05EB916DE784614291952C986867644F9711CD15050EB01ED7AAD`，清单 6/6 一致，Authenticode 为 `NotSigned`。前一提交 `07ac062` 另有本机安装/卸载、3.99 秒冷启动、零残留和完整已安装 E2E 历史基线。                                                                      | 当前未签名公共 Windows 自动化与制品核验通过；本机精确二进制基线属于前一提交 |
| Windows 正式签名                  | 当前 0.7 NSIS 的 Authenticode 状态为 `NotSigned`；`Cert:\CurrentUser\My -CodeSigningCert` 计数为 0，`CSC_LINK`/`CSC_KEY_PASSWORD` 均未配置；GitHub `release-signing` Environment 已存在，但 Environment Secret、保护规则、审批者和等待计时器均为空，仓库 Actions Secret 名称也为空                                                                                                                                                                  | 未满足                                                                      |
| macOS 正式签名与 notarization     | 配置包含 hardened runtime 和 DMG/ZIP 目标，但本机没有 macOS 原生环境；`MAC_CSC_LINK`、`MAC_CSC_KEY_PASSWORD`、`APPLE_API_KEY`、`APPLE_API_KEY_ID`、`APPLE_API_ISSUER`、`APPLE_ID`、`APPLE_APP_SPECIFIC_PASSWORD`、`APPLE_TEAM_ID` 均未配置；`release-signing` Environment 虽已存在但没有签名 Secret 或保护规则，因而没有本轮签名/notarization 与干净设备证据                                                                                        | 未满足                                                                      |
| 当前源码对应的 Linux x64 AppImage | CI run `31295938442` attempt 1 的 Linux job `93201306030` 在原生 runner 完成 JDK 21 Java 调试、AppImage 构建、运行时完整性、已打包应用 E2E 与 artifact 上传；公共 AppImage 为 213,150,176 字节，SHA-256 `5E858E4DDE29A38E753651B0F6FBDB8441F98A261FAADA3A7B516E008CF6A2D1`，清单 5/5 一致。                                                                                                                                                         | 满足“Linux 至少可构建”、当前产品树公共原生自动化与制品核验                  |
| 当前源码对应的 macOS 产物         | CI run `31295938442` attempt 1 的 macOS job `93201306014` 在 `macos-15` ARM64 完成真实 Java 调试、DMG/ZIP 构建、运行时完整性、已打包应用 E2E 与 artifact 上传；DMG 为 204,601,830 字节、SHA-256 `833F2B5387E0CC5ADB0D480068FD6CE198F92687127FC0F20CAE2F58CF342991`，ZIP 为 204,948,139 字节、SHA-256 `438082E57435D8B4BBAEEDC61711A14867D2E8E83382188470FF3A2DFC94633A`，清单 8/8 一致。                                                            | 当前产品树的原生自动化与制品核验满足；仍未签名/notarize、无独立人工设备证据 |
...
| 0.7 GitHub Release                | 远端最新 Release 为 0.6.0-alpha.1；本轮精确候选的四个必需 artifact 已上传、下载并核验，但 Windows 仍为 `NotSigned`，macOS 仍无签名/notarization 证据                                                                                                                                                                                                                                                                                                | 未满足；正式签名、notarization、真实 Provider 与独立验收完成前不发布        |
| 单个业务文件原则上不超过 400 行   | `pnpm source-size:check` 排除测试、产物和 vendor 后检查 353 个 TypeScript/TSX 生产文件，最大 400 行、超限 0；该门禁同时进入普通 CI 与正式发布工作流                                                                                                                                                                                                                                                                                                 | 满足                                                                        |
...
重跑前先按精确 ID 删除 96 个已被后续结果取代的旧 artifact；2026-08-08 又经用户明确授权删除 run `30933901524` 的 `8902424531`、`8902565841`、`8902573519`、`8902708321`，本地完整备份仍位于 `D:\release-evidence\open-code-desk-ci-30933901524`。随后经用户明确授权删除 run `30944352985` 的 `8906575235`、`8906664876`、`8906713192`、`8906833661`、`8912946359`；删除后 API 复核仓库 artifact 数为 0。旧 artifact 在 GitHub 上不可直接恢复，但对应本地证据均已保留。

最后 5 个旧 artifact 的删除前证据已补齐：四个发布 artifact 位于 `D:\release-evidence\open-code-desk-ci-30944352985`，Windows 6 项、macOS 8 项、Linux 5 项清单共 19/19 逐项重算一致，质量元数据 4/4 与 Linux 构建副本相同；诊断 artifact 已按远端名称重新下载到 `D:\release-evidence\open-code-desk-ci-30944352985-diagnostics-artifact`，其 4 个原始文件与已有故障证据逐项一致。仓库为 private；[GitHub 当前官方配额](https://docs.github.com/en/billing/reference/product-usage-included)按账户计划提供 500 MB（Free）或 1 GB（Pro）Actions 存储，且[存储使用量更新需要 6–12 小时](https://docs.github.com/en/billing/concepts/product-billing/github-actions)。等待官方重算窗口后，attempt 2 已成功上传总归档字节 804,337,666 的四个必需 artifact。
...
2026-08-09 在配额刷新后对同一 run 执行完整 attempt 2，最终结论为 `success`：Quality gates job `93169003177`、Windows job `93169391300`、macOS job `93169391317` 与 Linux job `93169391314` 全部成功。四个必需 artifact 均已上传：`quality-security-metadata`（ID `9029217996`，35,319 字节）、`open-code-desk-windows-x64`（ID `9029279003`，182,554,304 字节）、`open-code-desk-macos-arm64`（ID `9029249469`，409,105,328 字节）及 `open-code-desk-linux-x64`（ID `9029254648`，212,642,715 字节）。全部下载到新的证据目录 `D:\release-evidence\open-code-desk-ci-31247288230-attempt-2` 后，Windows 6/6、macOS 8/8、Linux 5/5，共 19/19 清单条目逐项重算一致，缺失、额外、重复与哈希不一致均为 0；质量元数据 4/4 与 Linux 副本逐字节一致。公共 Windows NSIS 的 Authenticode 仍为 `NotSigned`；普通 CI 未执行 macOS 正式签名与 notarization，因此制品齐备不等同于正式发布门槛齐备。

同日从精确提交 `07ac062` 的独立工作树全新构建 Windows NSIS，并在复制到独立证据目录后重新校验 `SHA256SUMS.txt` 6/6。对该安装包执行 CI 同款隔离安装烟测、已安装核心 E2E 9/9 和桌面壳/持久化 E2E 6/6 均通过；安装、关闭、卸载后目录与产品进程均为 0。随包依赖审计为 High 0 / Critical 0 / Moderate 1，许可证待复核 0，Secret 扫描 560 个源码文件且发现 0，SBOM 为 588 个组件。该本地证据补齐前一 Windows 产品树的精确二进制核验，但不替代当前公共 artifact、正式签名或独立人工验收。

随后发现此前“生产 TypeScript/TSX 超过 400 行为 0”的报告缺少确定性执行门禁，且当前工作树有 4 个物理行数超限文件。提交 `a3471ca14f883845b7a6bc3bacfef25dfa65d5ba` 将 Java/Node/Python DAP 与运行配置 UI 按职责拆分，并新增 `pnpm source-size:check`；本机检查 353 个生产文件、最大 400 行、超限 0，完整单元测试 394/394、集成测试 94/94、真实浏览器与 Electron 调试各 1/1、Electron E2E 16/16、构建、格式、类型、Lint、文档和 Secret 扫描全部通过。

该提交的公共 CI run `31295938442` attempt 1 最终为 `success`：Quality job `93200922695`、Windows job `93201306020`、macOS job `93201306014`、Linux job `93201306030` 全部成功。四个 artifact 为 `quality-security-metadata`（ID `9032978570`，35,319 字节）、`open-code-desk-windows-x64`（ID `9033037497`，182,554,211 字节）、`open-code-desk-macos-arm64`（ID `9033018291`，409,097,902 字节）和 `open-code-desk-linux-x64`（ID `9033013734`，212,643,137 字节）。全部下载到新目录 `D:\release-evidence\open-code-desk-ci-31295938442-attempt-1` 后，Windows 6/6、macOS 8/8、Linux 5/5，共 19/19 清单条目逐项重算一致；缺失、额外、重复、格式错误与哈希不一致均为 0，质量元数据 4/4 与 Linux 副本逐字节一致。依赖审计 High 0 / Critical 0 / Moderate 1，许可证待复核 0，Secret 扫描 567 个文件且发现 0，质量 SBOM 587 个组件。Windows NSIS 的 Authenticode 为 `NotSigned`，目标 tag/release 不存在；这些边界符合内部 Alpha，但不满足正式发布。

## 发布判定

当前精确候选已完成三平台构建、自动化与制品核验，可标记为受控内部 Alpha，并在内部测试范围内使用。它不是正式发布；本次不创建 `v0.7.0-alpha.1` Tag 或 GitHub Release，也不对外分发。Windows/macOS 正式签名与 notarization、全部计划 Provider 的真实公网通过记录，以及独立验收人员在干净设备上的完整 AI 编程与调试闭环记录均明确延期，在这些证据补齐前不得将本结论升级为正式发布就绪。
