...
- `319a25d277de1d610aae877b92c30452dd647518` 的 CI run `30914375533` 已在 Windows x64、macOS ARM64、Linux x64 完成真实 Java 调试、安装包构建、运行时完整性和已打包应用 E2E，四个 job 全部成功。
...
| 当前源码对应的 Windows 安装包     | CI run `30914375533` 从提交 `319a25d` 构建 0.7 NSIS，并完成运行时完整性、安装烟测和已安装应用 E2E；下载后的安装包为 182,382,737 字节，SHA-256 `B027E549F28F7360643AB861E1B8CEDDBB375C94D870924527ABFE34DE99ACC8`，重算值与随包 `SHA256SUMS.txt` 一致，Authenticode 状态为 `NotSigned`。先前本地候选另完成 9/9 核心 E2E、6/6 桌面壳/持久化 E2E、1.61 秒冷启动、正常关闭零残留与静默卸载。                                                            | 当前公共 CI 未签名候选通过                                                        |
...
| 当前源码对应的 Linux x64 AppImage | 本地 Debian 12 Docker 证据仍保留；公共 CI run `30914375533` 又在 Linux x64 原生 runner 完成 JDK 21 Java 调试、AppImage 构建、运行时完整性、已打包应用 E2E 与 artifact 上传。                                                                                                                                                                                                                                                                        | 满足“Linux 至少可构建”及公共原生 CI 自动化证据；仍非公开 Release/独立人工设备证据 |
| 当前源码对应的 macOS 产物         | CI run `30914375533` 在 `macos-15` ARM64 以 JDK 21 完成真实 Java 调试，生成 DMG/ZIP，验证 JDT LS、Java Debug Server、`node-pty` 原生 helper 的格式与执行权限，并完成已打包应用 E2E；artifact `open-code-desk-macos-arm64` 已上传。                                                                                                                                                                                                                  | 当前提交的原生自动化已满足；仍未签名/notarize，也没有独立人工设备证据             |
...
2026-08-04 使用当前 GitHub 身份重新只读查询：产品代码提交 `319a25d277de1d610aae877b92c30452dd647518` 已推送到远端 `release/0.7.0-alpha.1`；CI run `30914375533` 的 Quality gates、Windows x64、macOS ARM64、Linux x64 四个 job 均成功，并上传三平台产物与质量/安全元数据。最新 Release 仍为 `v0.6.0-alpha.1`，仓库 Environment 与 Secret 名称查询均为空。
