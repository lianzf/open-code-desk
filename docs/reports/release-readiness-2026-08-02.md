...
- `0b11ee06fed96d16fdd954477b16744caa97836b` 的 CI run `30922841285` 已在 Windows x64、macOS ARM64、Linux x64 完成真实 Java 调试、安装包构建、运行时完整性和已打包应用 E2E，四个 job 全部成功；四个 check-run annotations 为空。
...
CI run `30922841285` 的四个 artifact 已下载到独立证据目录
`D:\release-evidence\open-code-desk-ci-30922841285`。GitHub artifact 摘要对应服务端归档，
下表 SHA-256 对应解压后的实际安装/分发文件；二者不混用。

| Artifact                     | ID           | 归档字节    | GitHub 归档摘要                                                    | 到期时间（UTC）     |
| ---------------------------- | ------------ | ----------- | ------------------------------------------------------------------ | ------------------- |
| `open-code-desk-windows-x64` | `8898147144` | 182,552,945 | `b08c6df30497daf5bba9981db21478d8071897437c3c69b34a823275ea01ae0d` | 2026-08-18 15:20:54 |
| `open-code-desk-macos-arm64` | `8898107698` | 409,106,722 | `6aa282a199a9375c79336cfc46e44f902f26564acbfb1ce70f1635892cabff02` | 2026-08-18 15:19:35 |
| `open-code-desk-linux-x64`   | `8898071786` | 212,643,365 | `391200505d1a701885017a783e1e9b0a3dfea30c76a42a4e3e637be19a136635` | 2026-08-18 15:18:44 |
| `quality-security-metadata`  | `8897948617` | 35,208      | `15c71acbb9455d7d5710ff91deeb8869c46513e41e4e9f8ea058f27314820760` | 2026-08-18 15:15:16 |

| 分发文件                                    | 字节        | 重算 SHA-256                                                       | 清单比对 |
| ------------------------------------------- | ----------- | ------------------------------------------------------------------ | -------- |
| `OpenCode Desk Setup 0.7.0-alpha.1.exe`     | 182,382,846 | `D8A87D64FB79B69EF41BEC2130326E6EB02F3AA21391B69664C99BD57659750A` | 一致     |
| `OpenCode Desk-0.7.0-alpha.1.AppImage`      | 213,150,123 | `106D16AE4DF20E341A42E0782613D7A16409461A3039B731AA73C6CEDB0AAC25` | 一致     |
| `OpenCode Desk-0.7.0-alpha.1-arm64.dmg`     | 204,604,622 | `76566525E6B50CF11E5BAF394AB22ECBA0729504F9A4071EC0948B401B2D0592` | 一致     |
| `OpenCode Desk-0.7.0-alpha.1-arm64-mac.zip` | 204,947,199 | `69DA17131CC95B5F9D30801B3E41E3F2A3B719CBDEC9FDC5959A954FBE5DB2F0` | 一致     |
...
| 当前源码对应的 Windows 安装包     | CI run `30922841285` 从提交 `0b11ee0` 构建 0.7 NSIS，并完成运行时完整性、安装烟测和已安装应用 E2E；下载后的安装包为 182,382,846 字节，SHA-256 `D8A87D64FB79B69EF41BEC2130326E6EB02F3AA21391B69664C99BD57659750A`，重算值与随包 `SHA256SUMS.txt` 一致，Authenticode 状态为 `NotSigned`。先前本地候选另完成 9/9 核心 E2E、6/6 桌面壳/持久化 E2E、1.61 秒冷启动、正常关闭零残留与静默卸载。                                                            | 当前公共 CI 未签名候选通过                                                        |
...
| 当前源码对应的 Linux x64 AppImage | 公共 CI run `30922841285` 在 Linux x64 原生 runner 完成 JDK 21 Java 调试、AppImage 构建、运行时完整性和已打包应用 E2E；下载文件为 213,150,123 字节，SHA-256 `106D16AE4DF20E341A42E0782613D7A16409461A3039B731AA73C6CEDB0AAC25`，与清单一致。                                                                                                                                                                                                        | 满足“Linux 至少可构建”及公共原生 CI 自动化证据；仍非公开 Release/独立人工设备证据 |
| 当前源码对应的 macOS 产物         | CI run `30922841285` 在 `macos-15` ARM64 以 JDK 21 完成真实 Java 调试，生成 DMG/ZIP，验证 JDT LS、Java Debug Server、`node-pty` 原生 helper 的格式与执行权限，并完成已打包应用 E2E；DMG 与 ZIP 的重算 SHA-256 均与清单一致。                                                                                                                                                                                                                        | 当前提交的原生自动化已满足；仍未签名/notarize，也没有独立人工设备证据             |
...
2026-08-04 使用当前 GitHub 身份重新只读查询：产品代码提交 `0b11ee06fed96d16fdd954477b16744caa97836b` 已推送到远端 `release/0.7.0-alpha.1`；CI run `30922841285` 的 Quality gates、Windows x64、macOS ARM64、Linux x64 四个 job 均成功，并上传三平台产物与质量/安全元数据。此前 `cb7e6fd2720d3ff79cfd5a32b72a03b8cdc9e732` 的 run `30921479075` 在 Linux 已打包应用 E2E 中暴露 Python 单步后的旧暂停状态竞态；测试改为确认暂停位置推进至 `main.py:5` 后，当前 run 在本机 5 次重复测试及 Linux runner 均通过。最新 Release 仍为 `v0.6.0-alpha.1`，`v0.7.0-alpha.1` tag 不存在，仓库 Environment 与 Actions Secret 名称查询均为空。
