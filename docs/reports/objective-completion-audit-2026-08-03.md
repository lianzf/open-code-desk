...
日期：2026-08-03（Electron 双进程追加复验：2026-08-04）  
目标来源：`goal-objective.md`，1,664 行，SHA-256
`0BA834EC5E45CD7F86693D5E2B36F3CCAF6E36249D33B992F00A8B7D4E5AF038`

## 审计结论

当前工作树已经形成可安装、可运行、可配置、具备真实 AI 编程闭环和真实
Node.js/Python/浏览器/Electron 主渲染双进程 DAP 调试闭环的 Windows 0.7 alpha 候选，并完成 Java、C/C++/Rust、Go、.NET 的
真实 Windows DAP 验收。浏览器、Electron、JDT LS/Java Debug Server、LLVM/LLDB、Delve 与 NetCoreDbg 均已验证
源码断点、调用栈、局部变量、单步、退出和进程清理。当前工作树快照还在 Debian 12 Linux x64
容器内完成冻结依赖安装、原生 `node-pty` 编译、AppImage 打包、自解包、运行时完整性校验、GNOME
Secret Service 探测和 9/9 已打包应用核心 E2E。核心本地自动化、性能、安全静态门禁、Windows
安装后验收和 400 行结构约束均已通过。

项目仍不能标记为正式完成。以下硬证据尚缺失：

1. 当前 0.7 工作树尚未在 macOS 原生 runner 构建和运行；Linux 已有本地 Docker Linux x64 证据，macOS ARM64/Linux x64 打包作业也已配置真实 Java 调试验收，但尚未触发当前提交的公共原生 CI 或干净设备验收。
2. Windows Authenticode、macOS 签名与 notarization 未完成；本机代码签名证书为 0，Apple/CSC 签名输入均未配置；GitHub Environments 只读查询返回总数 0，`release-signing` 环境尚不存在。
3. 尚无干净 Windows/macOS/Linux 设备和独立验收人员的完整人工闭环记录。
4. 全部计划 Provider 的真实公网凭据连接未逐一验收；当前环境未配置 Provider 凭据。现已提供 10 个 Provider 的显式选择、60 秒超时、最小流式请求和失败脱敏门禁，但默认跳过结果不能替代提供真实凭据后的运行证据；现有通过证据仍只覆盖适配器、协议测试和本地真实 HTTP 流程。
5. Renderer 错误展示点已统一经过本地化边界；应用自有的主进程/DAP 英文与中文诊断均受双向源码覆盖测试约束，项目检测、命令风险和工具批准原因也有双语测试。第三方原始诊断按原文保留，因此产品自有动态诊断的完整双语要求已满足。

2026-08-04 使用当前 GitHub 身份重新执行只读核验：远端不存在 `release/0.7.0-alpha.1` 分支，本地仍基于 `75415e5a8421a6d5084c7001448918e4c073da94` 的大量未提交工作树；最新 CI 仍为 run `30723027223`，最新发布仍为 `v0.6.0-alpha.1`，仓库 Environment 总数仍为 0。未触发工作流、未创建分支、未修改远端。
...
| `pnpm build`                       | 通过                                                      |
| `pnpm test`                        | 111 个文件 / 380 项测试通过；7 个门禁文件 / 18 项默认跳过 |
...
| 目标域          | 判定                                 | 当前证据或缺口                                                                                                                                                                                                 |
| --------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 桌面端基础能力  | Windows/Linux 已证明；macOS 部分证明 | Windows NSIS 安装、启动、关闭、卸载通过；Linux x64 AppImage 在 Debian 12 容器完成原生构建、自解包、目标运行时/原生模块校验和 9/9 已打包应用 E2E；macOS 当前仅有构建配置与历史 CI 证据                          |
...
| API Key 安全    | 已证明 Windows/Linux 本地闭环        | `safeStorage`、密钥仓库、脱敏和安装版 Provider E2E；静态检查没有硬编码 `sk-…` 密钥；Linux GNOME Secret Service 已真实写入/读取/清除并完成已打包 E2E；macOS Keychain 当前源码仍待原生复验                       |
...
| 语言范围           | 本地语言闭环已证明；跨环境路线部分完成       | Node.js/TypeScript、Python、浏览器前端、Electron 主/渲染进程、Java、C/C++/Rust、Go、.NET 均完成 Windows 真实调试验收；Node Inspector 远程/容器附加已用真实独立目标验证，Java macOS/Linux、其他语言跨环境附加和自动隧道仍是后续范围           |

## 交付物审计

| 交付物                         | 判定                   | 位置或缺口                                                                                                                                             |
| ------------------------------ | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 完整源代码                     | 存在，尚未形成提交证据 | 当前工作树包含大量未提交变更，不能用远端 HEAD 代表当前候选                                                                                             |
| Windows 安装包                 | 已证明本地候选         | `release/OpenCode Desk Setup 0.7.0-alpha.1.exe`                                                                                                        |
| Linux x64 AppImage             | 已证明本地候选         | `.runtime/linux-validation/release-linux-x64/OpenCode Desk-0.7.0-alpha.1.AppImage`；忽略目录内的本地验收证据，不是公开 Release                         |
| macOS/Linux 构建配置           | 存在                   | `apps/desktop/electron-builder.yml`、目标平台运行时校验器、原生 CI 与签名发布 matrix；macOS ARM64/Linux x64 打包作业包含真实 Java 调试验收             |
...
| DoD                                    | 判定                                                     |
| -------------------------------------- | -------------------------------------------------------- |
| 核心业务闭环全部可运行、功能为真实实现 | Windows 自动化证据支持；独立人工验收尚缺                 |
| 用户可配置/切换模型，API Key 安全保存  | Windows/Linux 本地闭环已证明；macOS Keychain 待原生复验  |
| 文件受权限控制且修改可审核/回滚        | 已证明                                                   |
| 命令可授权、终止和持久化               | 已证明                                                   |
| 会话与任务可持久化                     | 已证明                                                   |
| 核心自动化、类型、Lint、构建通过       | 已证明当前本地源码                                       |
| Windows 安装包正常安装运行             | 已证明未签名候选                                         |
| 项目文档完整                           | 主要交付文档齐全                                         |
| 无阻塞严重缺陷/已知高危安全问题        | 当前自动化和依赖审计未发现；跨平台证据完成前不能最终确认 |
