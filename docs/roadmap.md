...
> 状态：IDE 扩展阶段 B（受管项目运行）已完成，阶段 C（Node.js/TypeScript DAP 调试）待开始
> 日期：2026-08-01
...
- [ ] **阶段 C**：Node.js/TypeScript DAP client、真实断点、线程、调用栈、变量、监视与单步控制。
- [ ] **阶段 D**：异常定位、调试上下文脱敏、AI 分析与 Diff 修复闭环。

阶段 B 的项目运行服务与 Agent 命令、用户交互终端相互独立；阶段 C 不得复用普通进程输出伪装 DAP 状态。
