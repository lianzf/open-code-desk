...
当前阶段版本：[`0.4.0-alpha.1`](docs/releases/0.4.0-alpha.1.md)
...
- 使用真实 Node.js Debug Adapter 设置/验证断点，查看线程、调用栈、局部变量和监视表达式
...
IDE 级项目识别、运行配置、受管进程和 Node.js/TypeScript DAP 调试已经形成真实闭环。调试状态来自
官方 js-debug 的协议事件，不从普通运行日志推断。下一阶段将把用户选择且脱敏后的异常、调用栈和
变量交给 AI，完成“定位 → 修复 Diff → 审批 → 重新调试”的辅助修复闭环。
...
- 条件断点、命中次数、日志断点、函数断点和数据断点尚未实现
- 调试上下文交给 AI 分析并自动编排重新验证仍在阶段 D 开发中
