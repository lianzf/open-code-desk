...
当前阶段版本：[`0.3.0-alpha.1`](docs/releases/0.3.0-alpha.1.md)
...
IDE 级项目识别、运行配置和受管进程运行已经形成独立闭环，DAP 调试仍在按独立模块建设。在真实断点、调用栈、变量和单步调试闭环完成前，本项目不会把普通运行或终端命令描述为 IDE 调试能力。
...
- DAP 断点调试、调用栈、变量、单步控制和 AI 调试上下文仍在开发中
