        ...
        <div className="min-h-0 overflow-auto border-r border-zinc-800">
          ...
          <DebugSection title="断点">
            {state.breakpoints.length === 0 ? (
              ...
              <div className="flex justify-end border-b border-zinc-900 px-2 py-1">
                <button
                  ...
                  onClick={() => void state.deleteAllBreakpoints()}
                >
                  清空项目断点
                </button>
              </div>
            ...
            {state.breakpoints.map((breakpoint) => (
              <div
                key={breakpoint.id}
                className="group flex w-full items-center gap-1 px-2 py-0.5 text-[10px] hover:bg-zinc-900"
                data-testid="debug-breakpoint-item"
              >
                <button
                  type="button"
                  className={`size-2.5 shrink-0 rounded-full border border-red-500 ${breakpoint.enabled && breakpoint.status === 'verified' ? 'bg-red-500' : ''} ${breakpoint.enabled ? '' : 'opacity-30'}`}
                  onClick={() =>
                    void state.setBreakpointEnabled(breakpoint.id, !breakpoint.enabled)
                  }
                  title={`${breakpoint.enabled ? '禁用' : '启用'}断点；当前状态：${breakpoint.status}${breakpoint.message === undefined ? '' : `，${breakpoint.message}`}`}
                  aria-label={`${breakpoint.enabled ? '禁用' : '启用'} ${breakpoint.relativePath}:${breakpoint.line}`}
                />
                <button
                  type="button"
                  className="min-w-0 flex-1 truncate py-0.5 text-left text-zinc-500"
                  onClick={() =>
                    void openFileAt(
                      workspaceId,
                ...
                <button
                  type="button"
                  className="shrink-0 p-0.5 text-zinc-700 opacity-0 hover:text-red-300 group-hover:opacity-100 focus:opacity-100"
                  onClick={() => void state.deleteBreakpoint(breakpoint.id)}
