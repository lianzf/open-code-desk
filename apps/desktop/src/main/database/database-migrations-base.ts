export const baseDatabaseMigrations = [
  `
    ...
    CREATE INDEX IF NOT EXISTS provider_configs_kind_idx ON provider_configs(kind);
    CREATE INDEX IF NOT EXISTS provider_configs_updated_at_idx ON provider_configs(updated_at);
  ...
  `
    ...
    CREATE INDEX file_change_sets_task_idx ON file_change_sets(task_id);
    CREATE INDEX file_change_sets_status_idx ON file_change_sets(status);
    ...
    CREATE INDEX file_changes_set_sequence_idx ON file_changes(change_set_id, sequence);
    CREATE INDEX file_changes_set_status_idx ON file_changes(change_set_id, status);
  `,
  `
    ...
    CREATE INDEX command_executions_status_idx ON command_executions(status);
  ...
  `
    ...
    CREATE INDEX model_configs_provider_updated_idx ON model_configs(provider_config_id, updated_at);
  ...
  `
    ...
    CREATE INDEX audit_events_conversation_created_idx ON audit_events(conversation_id, created_at);
  ...
  `
    ...
    CREATE INDEX crash_reports_created_idx ON crash_reports(created_at);
    CREATE INDEX crash_reports_acknowledged_idx ON crash_reports(acknowledged_at, created_at);
  `,
  `
    CREATE TABLE run_configurations (
      ...
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL,
      CONSTRAINT run_configurations_auto_generated_check CHECK (auto_generated IN (0, 1)),
      CONSTRAINT run_configurations_console_check CHECK (console IN ('integratedTerminal', 'runOutput')),
      CONSTRAINT run_configurations_type_check CHECK (type IN (
